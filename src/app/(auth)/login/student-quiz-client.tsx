...
import { getQuizQuestions, getLessonVideos, submitQuizResult } from "@/app/actions/student-quiz";
...
export function StudentQuizClient({ initialQuestions, initialVideos = [], returnPath }: Props) {
  ...
  useEffect(() => {
    ...
    if (currentGrade !== 4) {
      Promise.all([
        getQuizQuestions(currentGrade),
        getLessonVideos(currentGrade)
      ]).then(([qData, vData]) => {
        setQuestions(qData);
        setVideos(vData);
      });
  ...
  // Tải danh sách câu hỏi và video tương ứng khi chuyển khối lớp
  useEffect(() => {
    let active = true;
    async function loadGradeData() {
      ...
      try {
        const [qData, vData] = await Promise.all([
          getQuizQuestions(selectedGrade),
          getLessonVideos(selectedGrade)
        ]);
        if (active) {
          setQuestions(qData);
          setVideos(vData);
  ...
  async function handleSubmitQuiz() {
    ...
    await submitQuizResult(studentName, className, correctCount, questions.length);
            ...
            <section className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-sky-400 via-sky-500 to-indigo-600 p-6 text-white shadow-xl sm:p-10">
              ...
              <div className="relative z-10 space-y-6 max-w-2xl">
                ...
                <div className="flex flex-wrap gap-2 pt-2">
                  <span className="bg-white/15 px-3 py-1.5 rounded-xl text-xs sm:text-sm font-semibold backdrop-blur-sm">📚 Môn: Tin học</span>
                  <span className="bg-white/15 px-3 py-1.5 rounded-xl text-xs sm:text-sm font-semibold backdrop-blur-sm">🎓 Lớp: {selectedGrade}</span>
                  <span className="bg-white/15 px-3 py-1.5 rounded-xl text-xs sm:text-sm font-semibold backdrop-blur-sm">📖 Sách: Kết nối tri thức</span>
                  <span className="bg-white/15 px-3 py-1.5 rounded-xl text-xs sm:text-sm font-semibold backdrop-blur-sm">📝 Số câu: {questions.length} câu</span>
                  <span className="bg-white/15 px-3 py-1.5 rounded-xl text-xs sm:text-sm font-semibold backdrop-blur-sm">⏱️ Thời gian: 15 phút</span>
                </div>
              ...
              <div className="space-y-6 pt-4">
                ...
                <section id="video-lessons-submitted" className="space-y-4 pt-4">
                  <h2 className="text-xl font-extrabold text-slate-800">
                    🎬 Em xem video bài học để hiểu sâu hơn nhé:
                  </h2>
                  {videos.length === 0 ? (
                    <Card className="border-sky-100 shadow-sm rounded-3xl bg-white p-8 text-center text-muted-foreground font-normal">
